...
public class MazeController implements Order {
	...
	/**
	 * Sets maze driver based on the item picked 
	 * In Menu selection
	 * @param num
	 */
	public void setMazeCont(int num) {
		this.basic = new BasicRobot();
		basic.setMaze(this);
        ...
        case 0:
        		...
        		this.wizard.setRobot(basic);
        		this.wizard.setDistance(this.mazeConfig.getMazedists());
        		this.pledge = null;
        		this.wallfollower=null;
        		this.explorer=null;
        		break;
        case 1:
        		this.pledge = new Pledge(this);
        		this.pledge.setRobot(basic);
        		this.wallfollower=null;
        		this.explorer=null;
        		this.wizard = null;
        		break;
        case 2:
        		...
        		this.wallfollower.setRobot(basic);
        		this.wizard=null;
        		this.explorer=null;
        		break;
        case 3:
    			this.explorer = new Explorer();
    			this.explorer.setRobot(basic);
        		this.wallfollower=null;
        		this.pledge=null;
        		break;
        case 4:
	...
	public void switchToPlayingScreen() {
		...
		if(this.wizard != null) {
			try {
				this.wizard.drive2Exit();
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		}
		else if(this.pledge != null) {
			try {
				this.pledge.drive2Exit();
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		}
		else if(this.wallfollower != null) {
			try {
				this.wallfollower.drive2Exit();
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public void switchToRobotScreen(boolean cancelOrder) {
		//System.out.println("switchToTitleScreen: param == " + cancelOrder) ;
		if (cancelOrder) {
			factory.cancel();
		}
		setState(StateGUI.ROBOT_TITLE);
		notifyViewerRedraw() ;
	...
	protected void notifyViewerRedraw() {
		...
		while (it.hasNext())
		{
			...
			//MazePanel pan = panel.getBufferGraphics()
			panel.setGraphics();
			if (null == panel) {
	...
	protected void operateGameInPlayingState(UserInput key) {
		...
		case Left: // turn left
			...
			//driver.rotateRobotLeft();
			
			//System.out.println("current" + dx+"x"+"    "+dy+"y");
		//	if(driver != null) {
		//		driver.klKeys(key);
		//	}
		//	else {
			...
			rotate(1);
		//	}
			//System.out.println("after" + dx+"x"+"    "+dy+"y");
			break;
		case Right: // turn right
			//driver.rotateRobotRight();
			//System.out.println("current" + dx+"x"+"    "+dy+"y");
		//	if(driver != null) {
		//		driver.klKeys(key);
		//	}
		...
		case ReturnToTitle: // escape to title screen
			...
			//basic.setBatteryLevel(0); //escape infinite loop in a driver
			/*
			if (wizard != null) {
				wizard.Battery = 0;
			}
			if (wallfollower != null) {
				wallfollower.Battery = 0;
			}
			if (pledge != null) {
				pledge.Battery = 0;
			}
	...
	protected void setState(StateGUI newState) {
		...
		switch (state) {
		case ROBOT_TITLE:
			assert(newState == StateGUI.ROBOT_TITLE): msg;
			break;
		case STATE_TITLE:
